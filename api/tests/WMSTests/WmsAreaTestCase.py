import random
from django.urls import reverse
from rest_framework import status
from rest_framework.test import APITestCase
from account.models import User
from rest_framework.test import APIClient
from api.views.WMSViewSet import WmsAreaViewSet
from api.filters.WMSFilter import WmsAreaFilter
from api.tests.WMSTests import common
from WMS.models.warehouse import WmsArea
import logging

logger = logging.getLogger(__name__)


class WmsAreaTests(APITestCase):

    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_superuser(username='testuser', password='testpassword')
        self.client.force_authenticate(self.user)
        self.test_data = {'is_show': True, 'create_time': '1920-05-17 00:00:00', 'edit_time': '1930-06-12 00:00:00',
                          'creator': None, 'editor': None, 'number': 'YdJdRMtM0X', 'name': '李玲',
                          'warehouse': common.get_link(common.create_warehouse()), 'remark': '上海不同电话.'}
        self.update_data = {'number': 'ZBIGwAnSQ6', 'name': '张晶', 'remark': '报告完全准备.'}

        url = reverse('wmsarea-list')
        response = self.client.post(url, self.test_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(WmsArea.objects.count(), 1)
        # update_data 可以避开 datetime field
        key = random.choice(list(self.update_data.keys()))
        value = getattr(WmsArea.objects.get(), key)
        self.assertEqual(value, self.test_data.get(key))
        self.obj = WmsArea.objects.get()

    def test_get_wmsarea_list(self):
        url = reverse('wmsarea-list')
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_update_wmsarea(self):
        url = reverse('wmsarea-detail', args=[self.obj.id])
        response = self.client.patch(url, self.update_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.obj = WmsArea.objects.get()
        key = random.choice(list(self.update_data.keys()))
        value = getattr(WmsArea.objects.get(), key)
        self.assertEqual(value, self.update_data.get(key))

    def test_search(self):
        url = reverse('wmsarea-list')
        search_key = random.choice(WmsAreaViewSet.search_fields)
        k = self.update_data.get(search_key, None) if not self.update_data.get(search_key,
                                                                               None) else self.test_data.get(search_key,
                                                                                                             "")
        full_url = url + "?search=" + k
        response = self.client.get(full_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_filter(self):
        pass

    def tearDown(self) -> None:
        pass
